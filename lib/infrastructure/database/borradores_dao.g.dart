// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'borradores_dao.dart';

// **************************************************************************
// DaoGenerator
// **************************************************************************

mixin _$BorradoresDaoMixin on DatabaseAccessor<Database> {
  $ActivosTable get activos => attachedDatabase.activos;
  $CuestionarioDeModelosTable get cuestionarioDeModelos =>
      attachedDatabase.cuestionarioDeModelos;
  $CuestionariosTable get cuestionarios => attachedDatabase.cuestionarios;
  $BloquesTable get bloques => attachedDatabase.bloques;
  $TitulosTable get titulos => attachedDatabase.titulos;
  $CuadriculasDePreguntasTable get cuadriculasDePreguntas =>
      attachedDatabase.cuadriculasDePreguntas;
  $PreguntasTable get preguntas => attachedDatabase.preguntas;
  $OpcionesDeRespuestaTable get opcionesDeRespuesta =>
      attachedDatabase.opcionesDeRespuesta;
  $InspeccionesTable get inspecciones => attachedDatabase.inspecciones;
  $RespuestasTable get respuestas => attachedDatabase.respuestas;
  $RespuestasXOpcionesDeRespuestaTable get respuestasXOpcionesDeRespuesta =>
      attachedDatabase.respuestasXOpcionesDeRespuesta;
  $ContratistasTable get contratistas => attachedDatabase.contratistas;
  $SistemasTable get sistemas => attachedDatabase.sistemas;
  $SubSistemasTable get subSistemas => attachedDatabase.subSistemas;
}
